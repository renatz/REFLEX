...
def combine(predicted_path, grouth_truth_path):
    ...
    for item in gt_list:
        ...
        # match = re.search(r"(TRUE|MOSTLY-TRUE|HALF-TRUE|BARELY-TRUE|FALSE|PANTS-FIRE)\.?\s*Explanation:", predicted_text)
        # match = re.search(r"(true|mostly-true|half-true|barely-true|false|pants-fire)\.?\s*Explanation:", predicted_text)
...
def get_performance(FAST_CHAT_FORMAT, TEST_SET_PATH, BENCH_NAME, MODEL_NAME):
    ...
    macro_f1 = f1_score(y_true_filtered, y_pred_filtered, labels=classes, average='macro')
    precision_weighted = precision_score(y_true_filtered, y_pred_filtered, labels=classes, average='macro')
    recall_weighted = recall_score(y_true_filtered, y_pred_filtered, labels=classes, average='macro')    
    log.info(f"P\tR\tF\tACC")
    log.info(f"{precision_weighted:.4f}\t{recall_weighted:.4f}\t{macro_f1:.4f}\t{accuracy:.4f}")
